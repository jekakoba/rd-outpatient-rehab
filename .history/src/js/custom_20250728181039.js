...
setInterval(() => {
	...
	if (last) {
		...
		clone.style.transform = "translateY(0) scale(0.7)"; // Початок із малого розміру
		...
		newItems.forEach((item, index) => {
			const scale = 1 - (index * 0.1); // Зменшення масштабу на 0.1 на кожен крок
			const translateY = index * -20 + "px"; // Зсув вгору на 20px на кожен крок
			const opacity = 1 - (index * 0.2); // Зменшення непрозорості на 0.2 на кожен крок
			const zIndex = newItems.length - index; // Вищий z-index для передніх елементів
			...
			if (index === 0) {
				item.style.opacity = "1"; // Переконатися, що передня картка повністю непрозора
			}
		});
	...
	Flip.from(state, {
		...
		onEnter: (el) => {
			gsap.fromTo(
				...
				{ opacity: 1, transform: "translateY(0) scale(1)", z